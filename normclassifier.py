...
##    ((0.0 <= a) & (a <= 0.125)).sum()
    ...
    image_array_r_train=norm(im_array[:,:,0])
    image_array_g_train=norm(im_array[:,:,1])
    image_array_b_train=norm(im_array[:,:,2])
    image_array=np.array([image_array_r_train,image_array_g_train,image_array_b_train])
    image_stack=np.vstack((image_stack,image_array))    
...
##    im_array_test=im_array_test[:,:,3]
    image_array_r=norm(im_array_test[:,:,0])
    image_array_g=norm(im_array_test[:,:,1])
    image_array_b=norm(im_array_test[:,:,2])
    image_array_test=np.array([image_array_r,image_array_g,image_array_b])
    


##    im_array_test=np.reshape(im_array_test,(1,27648))
    image_stack_test=np.vstack((image_stack_test,image_array_test))
...
clf_neu = MLPClassifier(solver='lbfgs',alpha=1e-8)
...
new_array=clf_lind.predict(image_stack_test)
...
np.savetxt("final_withavg_norm.csv", next_array, delimiter=",",fmt='%i',header='number,type')
